...
| D4 | **The pool is a warm cache, not a load-bearing dependency** | Measured generation cost (§3) is an order of magnitude below the strategy doc's estimate. The pool still earns its place, but "New game" no longer needs bundled seed puzzles to avoid a spinner on anything but Evil. |
...
│   │   ├── Difficulty.swift        # the 5-rung ladder + specs
...
│   │   ├── SeedPuzzles.bin        # pre-generated fallback set (§6.3)
...
│   │   └── Services/              # PuzzlePool, Haptics, Sound, Notifications,
...
| expert / evil | — | — | ≤ 4.0 s (pool-only, never inline) |
...
`generator.go:142-145`; (2) widen the bundled seed set; (3) drop the Evil rung.
...
evil:   (.advanced,    .advanced,     17, 120)  // maximal carve, no practical floor
```

`maxTier` never exceeds `.advanced` on any rung, so **no shipped puzzle ever
requires guessing** — the differentiator, preserved.

**P2-6 — MEASURED, and the risk is real: Expert and Evil are the same rung.**
...
1. **Drop Evil; ship four rungs.** Honest, free, and reversible. Recommended for
   v1.
2. **Raise Expert's floor to ~28 clues** so it stops early and genuinely carries
   more clues than Evil. Cheap, but the rung would often fail to reach tier 4
   with that many clues, so the near-miss fallback would fire far more often.
   Needs measuring before trusting.
3. **Extend the rater with a chain-level technique** (XY-wing or swordfish) as a
   genuine tier 5, and let Evil require it. This is the only option that makes
   Evil mean something, and it is a scoped follow-on: a new technique, new
   fixture entries, and a re-run of the cross-check — not a Phase 2 expansion.

Pending that decision the five-rung enum stays in place; collapsing it is a
one-line change in `Difficulty.swift` plus display strings.
...
Expert and Evil map onto `first_hard_solve` / `speed_hard_20m` for continuity;
new rungs get *new* keys in a v2 set rather than redefining existing ones.
...
```swift
actor PuzzlePool {
    func take(_ difficulty: Difficulty) async -> GeneratedPuzzle
    func refill() async
    func persist() async
    func restore() async
}
...
Ports `pool.go`'s idea — a warm buffer with backpressure — with three phone-
specific changes:

1. **Target depth 3 per difficulty**, not the server's 16. One user, one game at
   a time.
2. **Persisted to disk** (`Application Support/pool.bin`) so a cold launch has
   puzzles instantly; refilled in a low-priority `Task` on launch and on
   `scenePhase == .background`.
3. **Thermal and battery aware** — refills pause under
   `ProcessInfo.processInfo.thermalState >= .serious` or Low Power Mode.

Given §3, easy/medium/hard can generate inline in the worst case; **expert and
evil are pool-only** and fall back to `SeedPuzzles.bin` (20 pre-generated per
difficulty, ~4 KB) if the pool is dry. Dailies bypass the pool entirely — they
are seeded and cached in SwiftData on first open.
...
| P2-3 | The five-rung `Difficulty` ladder | each rung rates in band over 100 samples |
| P2-4 | `DailyPuzzle` + golden-value test over ~10 pinned dates | same date → same puzzle, twice and across processes |
| P2-5 | `PuzzlePool` actor + disk persistence + thermal/battery gating | pool survives a cold launch |
| P2-6 | Clue-count distribution for expert vs evil (§4.5 risk) | decision recorded: keep, drop, or extend the rater |
| P2-7 | `SeedPuzzles.bin` generator task + bundled resource | first "New game" ever is instant |
...
| Generation too slow on older iPhones | Spinner on "New game" | Measured baseline (§3) makes this unlikely for easy/medium/hard. Pool + seed puzzles cover expert/evil. Fallback: lower `attempts` (path exists in `generator.go:142`). |
| Expert and Evil feel identical | A difficulty rung that means nothing | Measure the clue distribution at P2-6 and decide explicitly: keep, drop to four rungs, or extend the rater. |
